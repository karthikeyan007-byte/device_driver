...
// Tasklet function
static void etx_tasklet_function(struct tasklet_struct *tasklet) {
    printk(KERN_INFO "ETX Tasklet: Processing deferred work. IRQ Count = %d\n", irq_counter);
}

// Declare tasklet
static DECLARE_TASKLET(etx_tasklet, etx_tasklet_function);
...
static int __init etx_driver_init(void) {
    ...
    printk(KERN_INFO "Initializing ETX Interrupt Driver with Tasklet\n");
    ...
    printk(KERN_INFO "ETX Interrupt Driver with Tasklet Initialized Successfully!\n");
    return 0;
}
...
static void __exit etx_driver_exit(void) {
    ...
    printk(KERN_INFO "ETX Interrupt Driver with Tasklet Unloaded\n");
}
...
MODULE_DESCRIPTION("Interrupt Example Driver with Sysfs, IRQ Counter, and Tasklet");