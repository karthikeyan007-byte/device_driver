...
// Tasklet function to process IRQ in process context
static void etx_tasklet_function(unsigned long data) {
    printk(KERN_INFO "ETX Tasklet: Processing deferred work. IRQ Count = %d\n", irq_counter);
}

// Declare a dynamic tasklet structure
static struct tasklet_struct etx_tasklet;

// Interrupt Handler
static irqreturn_t irq_handler(int irq, void *dev_id) {
    irq_counter++;  // Increment counter
    printk(KERN_INFO "ETX Interrupt Handler: Interrupt Occurred! Count = %d\n", irq_counter);
    tasklet_schedule(&etx_tasklet); // Schedule the tasklet
    return IRQ_HANDLED;
}
...
static int __init etx_driver_init(void) {
    ...
    printk(KERN_INFO "Initializing ETX Interrupt Driver with Dynamic Tasklet\n");
    ...
    tasklet_init(&etx_tasklet, etx_tasklet_function, 0);

    printk(KERN_INFO "ETX Interrupt Driver with Dynamic Tasklet Initialized Successfully!\n");
    return 0;
}
...
static void __exit etx_driver_exit(void) {
    ...
    printk(KERN_INFO "ETX Interrupt Driver with Dynamic Tasklet Unloaded\n");
}
...
MODULE_DESCRIPTION("Interrupt Example Driver with Sysfs, IRQ Counter, and Dynamic Tasklet");
