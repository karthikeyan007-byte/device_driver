...
static struct workqueue_struct *etx_wq;
static struct work_struct etx_work;
...
static irqreturn_t irq_handler(int irq, void *dev_id) {
    ...
    queue_work(etx_wq, &etx_work); // Schedule workqueue function
    return IRQ_HANDLED;
}
...
static int __init etx_driver_init(void) {
    ...
    printk(KERN_INFO "Initializing ETX Interrupt Driver with Workqueue\n");
    ...
    // Create workqueue
    etx_wq = create_singlethread_workqueue("etx_wq");
    if (!etx_wq) {
        printk(KERN_ERR "ETX: Failed to create workqueue\n");
    ...
    printk(KERN_INFO "ETX Interrupt Driver with Workqueue Initialized Successfully!\n");
    return 0;
}
...
static void __exit etx_driver_exit(void) {
    ...
    printk(KERN_INFO "ETX Interrupt Driver with Workqueue Unloaded\n");
}
...
MODULE_DESCRIPTION("Interrupt Example Driver with Sysfs, IRQ Counter, and Workqueue");
