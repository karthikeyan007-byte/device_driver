...
// Declare a static work structure
static DECLARE_WORK(etx_work, etx_work_function);

// Interrupt Handler
static irqreturn_t irq_handler(int irq, void *dev_id) {
    irq_counter++;  // Increment counter
    printk(KERN_INFO "ETX Interrupt Handler: Interrupt Occurred! Count = %d\n", irq_counter);
    queue_work(system_wq, &etx_work); // Use the global workqueue
    return IRQ_HANDLED;
}
...
static int __init etx_driver_init(void) {
    ...
    printk(KERN_INFO "Initializing ETX Interrupt Driver with Static Workqueue\n");
    ...
    printk(KERN_INFO "ETX Interrupt Driver with Static Workqueue Initialized Successfully!\n");
    return 0;
}
...
static void __exit etx_driver_exit(void) {
    ...
    printk(KERN_INFO "ETX Interrupt Driver with Static Workqueue Unloaded\n");
}
...
MODULE_DESCRIPTION("Interrupt Example Driver with Sysfs, IRQ Counter, and Static Workqueue");
