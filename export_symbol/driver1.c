/* driver1.c - Exporting function and variable */
...
static ssize_t etx_read(struct file *filp, char __user *buf, size_t len, loff_t *off) { return 1; }
...
static int __init etx_driver_init(void) {
    if (alloc_chrdev_region(&dev, 0, 1, "etx_Dev1") < 0) return -1;
    cdev_init(&etx_cdev, &fops);
    if (cdev_add(&etx_cdev, dev, 1) < 0) return -1;
    pr_info("Device Driver 1 Inserted\n");
    return 0;
}
...
static void __exit etx_driver_exit(void) {
    ...
    pr_info("Device Driver 1 Removed\n");
}
